...
TASK(adbms1818_spi, TASK_STACK_1024) {
  ...
  while (true) {
    ...
    LOG_DEBUG("AUX A Read:  %02X %02X %02X %02X %02X %02X [PEC: %02X %02X]\r\n", rx_aux[0], rx_aux[1], rx_aux[2], rx_aux[3], rx_aux[4], rx_aux[5], rx_aux[6], rx_aux[7]);
    delay_ms(10);

    LOG_DEBUG("AUX B Read:  %02X %02X %02X %02X %02X %02X [PEC: %02X %02X]\r\n", rx_aux[8], rx_aux[9], rx_aux[10], rx_aux[11], rx_aux[12], rx_aux[13], rx_aux[14], rx_aux[15]);
    delay_ms(10);

    LOG_DEBUG("AUX C Read:  %02X %02X %02X %02X %02X %02X [PEC: %02X %02X]\r\n", rx_aux[16], rx_aux[17], rx_aux[18], rx_aux[19], rx_aux[20], rx_aux[21], rx_aux[22], rx_aux[23]);
    delay_ms(10);
