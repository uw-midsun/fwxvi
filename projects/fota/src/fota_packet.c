...
FotaError encode_packet(const Packet *packet, NetworkBuffer *buffer) {
    if (!packet || !buffer) {
      ...
      if (packet->payload_length > PAYLOAD_SIZE)     return FOTA_ERROR_OVERFLOW;
...
FotaError decode_packet(Packet *packet, NetworkBuffer *net_buf){
    ...
    if (network_buffer_read(net_buf, &type) != FOTA_ERROR_SUCCESS ||
      network_buffer_read(net_buf, &seq) != FOTA_ERROR_SUCCESS ||
      network_buffer_read(net_buf, &length_bytes[0]) != FOTA_ERROR_SUCCESS ||
  ...
  memcpy(&received_crc, &packet->payload.raw[packet->payload_length - 4], sizeof(uint32_t));
