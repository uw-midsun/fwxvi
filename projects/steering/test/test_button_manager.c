...
static const ButtonConfig configs[] = {
  { .gpio = { .port = 0, .pin = 0 }, .active_low = true, .debounce_ms = 2, .callbacks = { .rising_edge_cb = test_press_callback, .falling_edge_cb = test_release_callback } }
};
...
void test_short_bounce_no_trigger(void) {
  ButtonConfig long_db_cfg = {
    .gpio = { .port = 0, .pin = 0 }, .active_low = true, .debounce_ms = 3, .callbacks = { .rising_edge_cb = test_press_callback, .falling_edge_cb = test_release_callback }
  };
  button_manager_init(&manager, &long_db_cfg, 1);

  // bounce shorter than debounce_ms=3
  mock_gpio_states[0] = GPIO_STATE_LOW;
  button_manager_update(&manager);  // count=1
  mock_gpio_states[0] = GPIO_STATE_HIGH;
  button_manager_update(&manager);  // reset
  mock_gpio_states[0] = GPIO_STATE_LOW;
  button_manager_update(&manager);  // count=1
  mock_gpio_states[0] = GPIO_STATE_HIGH;
  button_manager_update(&manager);  // reset
