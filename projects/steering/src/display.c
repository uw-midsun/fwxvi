...
static StatusCode s_render_gui_step(void) {
  ...
  if (current_screen == GUI_SCREEN_DRIVE) {
    ...
    status_ok_or_return(gui_drive_screen_widget_set_brake_bar(display_data->brake_enabled ? 100 : 0));  // TODO change to % base when available
    ...
    if (steering_storage->display_data.drive_state == VEHICLE_DRIVE_STATE_REGEN) {
      gui_drive_screen_widget_set_brake_bar_color(GUI_COLOR_REGEN_BRAKE_FILL);
    } else {
      gui_drive_screen_widget_set_brake_bar_color(GUI_COLOR_BRAKE_FILL);
    }
