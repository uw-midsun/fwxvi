 ...
 */
static Diskio_drvTypeDef sd_disk_driver = {
...
static DSTATUS sd_disk_initialize(BYTE pdrv)
{
    ...
    return (sd_card_init(s_spi_port, s_settings) == STATUS_CODE_OK)
         ? RES_OK : STA_NOINIT;
}
...
static DSTATUS sd_disk_status(BYTE pdrv)
{
    ...
    return (sd_is_initialized(s_spi_port) == STATUS_CODE_OK)
         ? RES_OK : STA_NOINIT;
}
...
static DRESULT sd_disk_read(BYTE pdrv, BYTE *buff, LBA_t sector, UINT count)
{
    ...
    return (sd_read_blocks(s_spi_port, buff, sector, count) == STATUS_CODE_OK)
         ? RES_OK : RES_ERROR;
}
...
static DRESULT sd_disk_write(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count)
{
    ...
    return (sd_write_blocks(s_spi_port, (uint8_t *)buff, sector, count) == STATUS_CODE_OK)
         ? RES_OK : RES_ERROR;
}
