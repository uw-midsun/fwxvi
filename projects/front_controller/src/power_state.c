...
static void set_load_switches(){
    ...
    uint8_t drive_state_from_rear = get_steering_buttons_drive_state();
    
    if (bps_fault_from_rear == 1){
        frontControllerStorage->current_power_state = POWER_STATE_FAULT;
    }else if (drive_state_from_rear == DRIVE_STATE_DRIVE || drive_state_from_rear == DRIVE_STATE_CRUISE || drive_state_from_rear == DRIVE_STATE_REVERSE){
        frontControllerStorage->current_power_state = POWER_STATE_ENGAGED;
    }else if(drive_state_from_rear == DRIVE_STATE_NEUTRAL){
