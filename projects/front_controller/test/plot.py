...
X_COL = 1          # pedal input (normalized)
Y_COL = 2          # accel percentage
...
for speed in sorted(groups.keys()):
    ...
    for i in range(len(pts) - 1):
        ...
        # Only plot segments where state stays the same (optional)
        # If you want to color by starting point's state only, remove this check
        # if s0 != s1:
        #     continue

        color = 'green' if s0 == 0 else 'red'
        alpha = 0.2 + 0.8 * speed  # opacity from 0.2 (speed=0) to 1.0 (speed=1)
...
# Create a manual legend for state colors
