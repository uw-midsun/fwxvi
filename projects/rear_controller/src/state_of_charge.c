 ...
 */
void estimate_initial_state_c(const float *v_samples, int n_samples,
                              int N_series,
                              const float *SOC_table, const float *OCV_table, int table_size,
                              float x0[2]) {
    ...
    float v_min = v_samples[0];
    float v_max = v_samples[0];
    float v_sum = 0.0f;
...
StatusCode state_of_charge_init(RearControllerStorage *storage) {
    ...
    memset(rtU.x_prev, 0, sizeof(rtU.x_prev));
    memset(rtU.P_prev, 0, sizeof(rtU.P_prev));
    ...
    rtU.params.Q_proc[0] = SOC_Q_PROC_DIAG_0;
    rtU.params.Q_proc[1] = 0;
    rtU.params.Q_proc[2] = 0;
    rtU.params.Q_proc[3] = SOC_Q_PROC_DIAG_3;
