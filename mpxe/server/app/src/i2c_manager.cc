...
void I2CManager::loadI2CInfo(std::string &projectName) {
  ...
  // Store i2c info from the I2C json
}
  ...
  try {
    switch (commandCode) {
      case CommandCode::I2C_SET_DATA: {
        ...
        if (portNum >= 3) { /* Assuming 3 I2C ports: I2C1, I2C2, I2C3 */
          throw std::runtime_error("Invalid selection for I2C ports. Expected: I2C1, I2C2, or I2C3");
          break;
        }
      ...
      case CommandCode::I2C_GET_DATA: {
        ...
        /* Set buffer length for the request */
        if (!data.empty()) {
          m_I2CDatagram.setBufferLength(data.size());
        }
