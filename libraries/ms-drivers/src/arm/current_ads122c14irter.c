...
/* Writes to registers : 0h05 to 0h0F*/
static StatusCode ads122_write_all_registers(ADS122Storage *storage, uint8_t data[], ADS122C14ITER_Register reg[]) {
  if (storage == NULL) {
    return STATUS_CODE_INVALID_ARGS;
  }

  uint8_t data_length = 11U;
  uint8_t tx_data[11U * 2];
