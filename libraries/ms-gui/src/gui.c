...
/* !Could move to a <common>.h type file maybe */
...
StatusCode gui_progress_bar(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t percentage, ColorIndex color_index_bg, ColorIndex color_index_fg) {
  if (color_index_bg >= NUM_COLOR_INDICES || color_index_fg >= NUM_COLOR_INDICES) {
    return STATUS_CODE_INVALID_ARGS;
  }

  uint16_t filled_width = (width * (uint16_t)percentage) / 100;
  for (uint16_t row = 0; row < height; ++row) {
    status_ok_or_return(gui_draw_line(x, y + row, x + filled_width - 1, y + row, color_index_fg));
    status_ok_or_return(gui_draw_line(x + filled_width, y + row, x + width - 1, y + row, color_index_bg));
  }
  return STATUS_CODE_OK;
}
