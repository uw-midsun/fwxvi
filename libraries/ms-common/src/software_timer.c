...
void timer_callback_wrapper(void *context) {
  SoftTimer *timer = (SoftTimer *)context;
  if (timer && timer->callback) {
    timer->callback(timer->context);
  }
}

StatusCode soft_timer_init(uint32_t duration_ms, SoftTimerCallback callback, void *context, SoftTimer *timer) {
  if (timer->id != NULL) {
  ...
  timer->id = xTimerCreateStatic(NULL, pdMS_TO_TICKS(duration_ms), pdFALSE, 
                                 (void *)timer,
                                 timer_callback_wrapper, &timer->buffer);
