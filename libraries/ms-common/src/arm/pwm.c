...
StatusCode pwm_init(PwmTimer timer, uint16_t period_us) {
  ...
  switch (timer) {
    ...
    case PWM_TIMER_3:
#ifdef STM32L4P5xx
      s_timer_handle[timer].Instance = TIM3;
#endif
      break;
    case PWM_TIMER_4:
#ifdef STM32L4P5xx
  ...
  if (timer == PWM_TIMER_2 || timer == PWM_TIMER_3 || timer == PWM_TIMER_4) {
...
StatusCode pwm_set_pulse(PwmTimer timer, uint16_t pulse_width_us, PwmChannel channel, bool n_channel_en) {
  ...
  if (timer == PWM_TIMER_2 || timer == PWM_TIMER_3) {
    /* General-purpose timers have no complementary (CHxN) outputs */
