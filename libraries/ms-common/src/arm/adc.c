...
StatusCode adc_init(void) {
  ...
  PeriphClkInit.AdcClockSelection = RCC_ADCCLKSOURCE_SYSCLK; // Or RCC_ADCCLKSOURCE_PLL
  HAL_RCCEx_PeriphCLKConfig(&PeriphClkInit);

  __HAL_RCC_ADC1_CLK_ENABLE(); // For ADC1
  __HAL_RCC_DMA1_CLK_ENABLE(); // For DMA1


  // DMA initialization needed for multi-channel scan
  DMA_DeInit(DMA1_Channel1); //change to HAL_DMA_Init(DMA1_Channel1);
